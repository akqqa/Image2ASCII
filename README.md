...
- Allows an image (JPG/PNG) or a GIF to be converted into ASCII art.
...
- Allows the size of the output image/gif to be chosen in terms of width (how many characters wide).
- Allows the aspect ratio of text characters to be input, to ensure the output image/gif is not warped in the chosen display font.
- Allows the image/gif to be inverted before conversion.
...
```g++ image2ASCII.cpp core.cpp -std=c++17 -O2 -o image2ASCII -ljpeg -lpng -lX11 -lboost_program_options -lgif```
...
```image2ASCII <path/to/image-or-gif>```
